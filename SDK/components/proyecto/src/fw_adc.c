...
/* Returns the adc channel and unit in which is the specific pin or -1 in case 
    ...
    */
static adc_pin_t fw_adc_pin_verification(int pin)
{
    adc_pin_t adc_pin;
    if (pin<0){
        adc_pin.unit=-1;
...
/* ---------------------------- Public API ---------------------------------- */
bool fw_adc_enable(int pin)
{
    adc_pin_t adc_pin=fw_adc_pin_verification(pin);
...
int fw_adc_read_voltage(int pin)
{
    int adc_raw_value=-1;
    adc_raw_value=fw_adc_read(pin);
    if(adc_raw_value==-1)
        return -1;
    else{
