...
bool fw_esp32_wifi_setup_ap(char *wifi_ssid, char *wifi_pass)
{
    ...
    if(ret!=ESP_OK)
	    return false;
    fw_event_post(FW_EVENT_WIFIAP, NULL);
	return true;
}
...
bool fw_esp32_wifi_connect(char *wifi_ssid, char *wifi_pass)
{
    ...
    fw_event_post(FW_EVENT_WIFICON, NULL);
    return true;
}

bool fw_esp32_wifi_disconnect(void)
{
	esp_err_t ret;
	ret=esp_wifi_disconnect();
	if(ret!=ESP_OK)
	    return false;
    fw_event_post(FW_EVENT_WIFIDIS, NULL);
	return true;
}

void fw_esp32_wifi_stop(void)
{
	esp_wifi_stop();
	esp_wifi_deinit();
}