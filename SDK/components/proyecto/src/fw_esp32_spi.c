...
bool fw_esp32_spi_initialize(int master_input_pin, int master_output_pin, int spi_clock_pin, int spi_cs_pin, int clk_out_mhz, spi_device_handle_t spi)
{
...
bool fw_esp32_spi_remove(spi_device_handle_t spi)
{
	if(spi==NULL)
		return false;
	esp_err_t ret;
...
int fw_esp32_spi_transmit(spi_device_handle_t spi, const uint8_t *data, int len)
{
	if(spi==NULL)
		return -1;
    esp_err_t ret;
    ...
    if(ret==ESP_OK){
        fw_event_post(FW_EVENT_SPITRANSMIT, NULL);
    	return len*8;
    }
    else
    	return -1;
