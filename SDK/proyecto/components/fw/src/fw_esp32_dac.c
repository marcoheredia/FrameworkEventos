...
static int fw_esp32_dac_pin_verification(int pin)
{
    if (pin<0){
        return -1;
    }
...
/* ---------------------------- Public API ---------------------------------- */
bool fw_esp32_dac_enable(int pin)
{
    int dac_channel=fw_esp32_dac_pin_verification(pin);
    if (dac_channel==-1)
        return false;
    esp_err_t err;
...
bool fw_esp32_dac_disable(int pin)
{
    int dac_channel=fw_esp32_dac_pin_verification(pin);
    if (dac_channel==-1)
        return false;
...
bool fw_esp32_dac_output_voltage(int pin, int mv_value)
{
    int dac_channel=fw_esp32_dac_pin_verification(pin);
