...
int8_t static fw_gpio_read_out(int pin)
{
...
{
    bool condition=fw_gpio_set_mode(pin, GPIO_MODE_INPUT);
    if (condition==false)
        return false;
    if(pull==GPIO_PULLUP_ONLY || pull==GPIO_PULLDOWN_ONLY ||
       pull==GPIO_PULLUP_PULLDOWN || pull==GPIO_FLOATING ){
        return fw_gpio_set_pull(pin,pull);
    }
    return false;
}
...
bool fw_gpio_setup_output(int pin, int8_t level)
{
    ...
    bool condition=fw_gpio_set_mode(pin, GPIO_MODE_OUTPUT);
    if (condition==false)
        return false;
...
{
    int8_t value=fw_gpio_read_out(pin);
    if(value==0)
        return fw_gpio_write(pin, 1);
    else if(value==1)
        return fw_gpio_write(pin, 0);
    return false;
}
